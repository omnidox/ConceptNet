...
def sorting_key(path_info):
    return path_info[2], -path_info[1]
...
def get_object_context(data, object_name, desired_contexts=None, focus_contexts=[]):
    ...
    sorted_paths = sorted(relevant_paths, key=sorting_key)
...
def move_down2(rot, trans):
    ...
    obj_pose.position.x = (trans[0])
    obj_pose.position.y = (trans[1])
    obj_pose.position.z = (trans[2]+ .030) #might need to be adjusted for certain items .038
    obj_pose.orientation.x = rot[0]
    obj_pose.orientation.y = rot[1]
    obj_pose.orientation.z = rot[2]
    obj_pose.orientation.w = rot[3]
    ...
    if fifth_joint > .03:
        ...
        prev_point = waypoints[-2]
        end_effector_pose = Pose()
        end_effector_pose.position.x = (end_effector_point.position.x - .08)#.05
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        for object in child_frame[:]:
            ...
            listener.waitForTransform('panda_link0', 'panda_hand', rospy.Time(), rospy.Duration(6.0))
            ...
            listener.waitForTransform('panda_link0', object,rospy.Time(), rospy.Duration(6.0))
